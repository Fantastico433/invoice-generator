...
jest.mock('./pdfExport', () => ({
  exportDocumentPdf: jest.fn(() => Promise.resolve()),
}));

import App from './App';
import { exportDocumentPdf } from './pdfExport';
...
test('exports a text-based PDF independently of live preview scale', async () => {
  render(<App />);
  ...
  expect(exportDocumentPdf).toHaveBeenCalledWith(
  ...
  expect(exportDocumentPdf.mock.calls[0][0].data.invoiceNumber).toBe('2025043002');
});
